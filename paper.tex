...
\begin{itemize}
  \item Attribute evaluator: This is the method by which the feature subsets are assessed. In this work, both a correlation-based feature subset evaluation method (CfsSubsetEval) and an adaptation of relief for attribute estimation (ReliefFAttributeEval) evaluation method \cite{RobnikSikonja1997} are used. 
...
``Relief'' is the second evaluator used in this work.
  \item Search method: This describes the structured way in which the set of possible feature subsets is studied, based on the results of the evaluator. Two search criteria have been used in this work: BestFirst, that searches the space of feature subsets by greedy hill-climbing augmented with a backtracking facility, and Ranker, that ranks features by their individual evaluations.
\end{itemize}
