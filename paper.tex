...
in different fields can be found. % frase que no dice nada. ¿Por qué no empiezas enmarcando el estado del
                                % arte? Methods to predict book sales have been generally based on ... - JJ
                                % [Pedro] no se encuentra gran cosa sobre predicción de ventas de libros
                                %  por eso quiero enfocarlo desde el punto de vista de la predicción 
                                %  de ventas en general, que sí hay bastante en muchos campos.
On the one hand, several qualitative methods are based on an expert
experience, while time series prediction methods \cite{Chu2003,Brown1959,Winters1960,Box1969,Papalexopoulos1990} 
are based on using historical data to make predictions
...
Although using time series prediction methods is perhaps the most used
technique to tackle the sales forecasting task, the efficiency of these 
techniques depends largely on the field of application and the problem data. 
Specifically, techniques that require a large amount of data for predicting 
sales of new products are not the most suitable for this task \cite{ChingChin2010}.

Thus, in the last years, different analysis and forecasting methods 
have been proposed for sales forecasting in several industry fields. %similar to the publishing industry.


  %[Pedro] el caso de la industria de la moda es muy similar al de la 
  % publicación de nuevos libros, ya que no hay datos históricos 
  % de un nuevo producto, y hay que echar mano de productos similares
  % o simplemente de los datos que describen el nuevo producto.
In the fashion industry, very similar to the publishing industry, 
predicting sales is a very complex problem, subject
to certain specific requirements \cite{SThomassey2014}.
This task has been faced using various methods in recent decades
  %[Pedro] me falta describir las aproximaciones
...
Finally, in many cases the predictions are influenced by external 
variables that must be taken into account, such as seasonality, 
promotions or fashions \cite{Lapide1999,ChingChin2010,ChernWSF15}.
In this sense, some authors propose reducing variations in the data 
caused by such variables to improve the accuracy of predictions
...
Taking into account the kind of problem we address in this paper, the 
classical forecasting methods are not useful. 
Thus, we must analyze raw data using other tools, prior to apply 
...
models based on rules are proposed.
