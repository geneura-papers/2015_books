...
\begin{table}
\caption{Predicting Total sales for all publishers (all the features). 30 repetitions, 10-fold cross validation.
...
information related to categorical variables, such as \emph{publisher} and 
\emph{collection}.
...
An important preprocessing step has been carried out before conducting the SOM analysis, so the two categorical features \emph{publisher} and \emph{collection} have been
...
Thus, we have run the same experiments as in Section \ref{subsec:results_all_publishers} with the whole dataset (all the publishers), but considering the selected features by each method. The results are shown in Table
...
\begin{table}
\caption{Predicting Total sales for PublishingHouse-UDL929 (The best selling publishing house) }
\centering{}%
\begin{tabular}{|c|c|c|c|c|c|c|}
\hline 
 & $R^{2}$  & MAE  & RMSE  & RAE (\%)  & RRSE (\%)  & Time (s)\tabularnewline
...
PublishingHouse704 with ReliefFAttributeEval feature selection.
...
PublishingHouse-UDLU11 with ReliefFAttributeEval feature selection
...
PublishingHouse-UDL929 with ReliefFAttributeEval feature selection.
...
PublishingHouse-UDLW12 with ReliefFAttributeEval feature selection.
