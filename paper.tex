...
Then, a Feature Selection (FS) process  has
...
(non-useful) features \cite{Krishnapuram2004}.
...
Preprocessing a dataset means that it is revised in order to reduce errors, such as wrong or duplicated patterns, or for completing missing values. The aim is to look for the correctness,
...
After this process, the features describing every pattern are frequently analysed.
In this case, among the variables describing a new book, some may affect 
predictions as they provide information, while others may not be 
...
% target? - JJ
Additionally, the algorithm running time grows with the number of
...
(SOM) \cite{kohonen1998} method has been also applied as a `subjective' (based on expert's opinion) feature selection technique based on the visualisation of the data as a set of 2D planes (one per feature of the dataset), and their interpretation for finding out relationships between these features.
...
With respect to feature selection methods, the used Weka software separates the process into two parts:
