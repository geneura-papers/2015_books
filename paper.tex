...
first year, reprints, units distributed as novelty, weeks for sale, total sales}.
...
\hline
  M5P 		& 23.13 $\pm$ 4.22    & 197.48  & 577.28  & 35.21    & 41.85  \\
\hline
  LR 		  & 3327.87 $\pm$ 607.58& 247.22 & 561.67  & 44.08    & 40.71    \\
\hline
  SVM 		& 5425.68 $\pm$ 990.59& 215.69  & 574.76  & 38.46    & 41.66  \\
\hline
...
forecasting methods on the dataset with X variables per book (obtained 
...
\hline
  kNN     & x $\pm$ y     & x  & x  & x    & x  \\
\hline
  MLP     & x $\pm$ y   & x $\pm$ y & x $\pm$ y  & x $\pm$ y  & x $\pm$ y   \\
\hline
\end{tabular}
\end{center}
\caption{\small{Results (errors and time) obtained using the different 
forecasting methods on the dataset obtained after the SOM analysis.}}
\label{tabla:resultsSOM}
\end{table*}


Obtained results (time to build the model and error) using the different 
forecasting methods on the dataset with 9 variables per book (obtained 
...
\end{center}
\caption{\small{Results (errors and time) obtained using the different 
forecasting methods on the dataset with 9 variables per book.}}
...
\end{center}
\caption{\small{Results (errors and time) obtained using the different 
forecasting methods on the dataset with 4 variables per book.}}
\label{tabla:results4}
\end{table*}

\begin{figure}[!h] 
\begin{center}
  \epsfig{file=fig_time.eps,width=7cm}
\caption{\small{Running time in seconds, obtained after applying the five methods to the reduced datasets (16, 9, 4 and X features each one). Plot shows that using categorical variables as part of the dataset makes the problem hard to solve. As was expected, simpler methods are much faster to build the model than the complex ones.}}
\label{fig:time}
\end{center}
\end{figure}
...
\begin{figure}[!h] 
\begin{center}
  \epsfig{file=fig_rae.eps,width=7cm}
\caption{\small{Relative absolute error (RAE $\%$) obtained after applying the five methods to the reduced datasets (16, 9, 4 and X features each one). The M5P model obtains the lowest errors, whatever the dataset used.}}
\label{fig:rae}
\end{center}
\end{figure}
