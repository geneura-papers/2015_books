...
%Después de esto no sé si merece la pena dejar este párrafo.
Forecasting sales of new books is a problem with issues 
such as dealing with limited data, finding adequate forecasting methods and 
selecting the best method to use \cite{ChingChin2010}. % He cambiado
                                % esto, pero sigue sin decir
                                % absolutamente nada. "The problem"
                                % tiene que describir el problema: qué
                                % queremos hacer, qué tenemos - JJ


% Este párrafo o se borra o se traslada al estado del arte. Aquí hay
% que contar cuál es el problema y qué se tiene. No se puede volver a
% repetir lo de antes.
...
%FERGU: data es plural. is->are   ...a mí me suena bien con "is"   ¡una segunda opinión!   ;)
In these cases not any forecasting method can be used \cite{Madsen2008,ChernWSF15}. 

% Vale, más issues. Este párrafo también puede quedar fuera. Ya has
% dicho que no se puede usar la serie histórica.
An additional issue is the difficulty to compare our model with classical 
...
and distributors. % Esto tampoco viene a cuento. Se dice en la intro o
                  % se cuenta como gente que nos ha proporcionado los
                  % datos y nos contrató para hacerlo.



Trevenque company, whose expert manager advised this research,
provided %FERGU: los párrafos anteriores son un poco dispersos, puedes
         %quitar los puntos y aparte y agruparlos por temas (por
         %ejemplo, todo lo de Trevenque debería de ir en un párrafo) 
% O borrarlos - JJ
%% Fin de párrafos a eliminar.
This initial data set, provided in the framework of a contract by
Trevenque Editorial, included 3159 books, and included descriptive information for each book includes price, 
points of sale, subject, publisher, sales, returns, among other information.
...
TOTAL DISTRIBUTED
DISTRIBUTED THROUGH DEPARTMENT STORES
DISTRIBUTED THROUGH DELEGATES
DISTRIBUTED - REST
TOTAL DISTRIBUTED FIRST YEAR
DISTRIBUTED THROUGH DEPARTMENT STORES FIRST YEAR
DISTRIBUTED THROUGH DELEGATES FIRST YEAR
...
NUMBER OF REPRINTS
TOTAL RETURNS
RETURNS - DEPARTMENT STORES
DELEGATES RETURNS
REST OF RETURNS
TOTAL RETURNS FIRT YEAR
RETURNS -  DEPARTMENT STORES  FIRST YEAR
DELEGATES RETURNS FIRST YEAR
REST OF RETURNS FIRST YEAR
GIFTS
UNITS DISTRIBUTED AS NOVELTY
TOTAL POINTS OF SALE
POINTS OF SALE - DEPARTMENT STORES
DELEGATES POINTS OF SALE
REST OF POINTS OF SALE
TOTAL POINTS OF SALE FIRST YEAR
DEPARTMENT STORES POINTS OF SALE FIRST YEAR
DELEGATES POINTS OF SALE FIRST YEAR
REST OF POINTS OF SALE FIRST YEAR
WEEKS FOR SALE
...
the forecasts, such as author, price, subject or publisher. %This has
                                %to be grounded by the experiment and
                                %graphs. 
...
Thus, in this study we propose three main stages (see Figure \ref{fig:stages} %FERGU: there are->we propose
for details): 
\begin{itemize}
  ...
  method to remove noise features \cite{Balaji2004}. The idea of these 
...
with high training sample concentration and fewer where the samples are scarce. %FERGU: mencionarlo en la intro


As stated in Section \ref{sec:problem}, an expert sales manager collaborated in 
...
\begin{itemize}
  ...
  and \emph{materia}. Thus, in a new iteration, taking into account the expert's %FERGU: materia no existe en inglés. Subject?
  advice and the information obtained analyzing the SOM results, the number of 
  inputs was decreased to 7.
  \item In a third step, several forecasting methods were used on the obtained 
  dataset, and the best was chosen to obtain an accurate predicting model.
...
describing each book to remove those that were superfluous.
...
\end{center}
\end{table}
%FERGU: para ahorrar espacio se puede hacer la tabla inversa: variables que se han quitado (las 7)
%[pedro] no entiendo muy bien a qué te refieres...

Then, after analyzing the original dataset using the SOM to select the best %FERGU: parametros o framework SOM usados?
...
Figure \ref{fig:som43} shows the initial analysis of dataset using the total 
...
\end{tabular}
\caption{\small{Initial data analysis, using 43 descriptive variables per book, %FERGU: la figura está en español, pero ya lo sabréis :P
using SOM.}}
...
Finally, after analyzing the dataset with 13 descriptive inputs per %FERGU: describis los resultados antes de explicar que volveis a hacer SOM
...
\emph{EDITORIAL} and \emph{SUBJECT} were included, while %FERGU: vale, no useis SUBJECT, usad mejor SUBJECT (SUBJECT es materia de física, creo xD).    ok, ¡gracias!
\emph{TOTAL DEVOLUTIONS, REPRINTS, TOTAL SELLING POINTS FIRST YEAR,
  ...
  WEEKS FOR SALE, PRINT-RUN} were discarded.  
...
Figure \ref{fig:som13} shows the initial analysis of dataset using the SOM on 
...
\end{tabular}
\caption{\small{Analysis of the initial data, using 13 descriptive variables per book, using the SOM.}} % Análisis de los datos iniciales, usando 13 variables descriptivas por libro, utilizando el SOM.}} %FERGU: teniais esto en español, lo he traducido al inglés :)
...
variables, two categorical variables, \emph{EDITORIAL} and \emph{SUBJECT}, are 
included, what that makes harder to process the instances and build the different 
models.

  %[pedro] ya están todos los resultados de de tabla:results7 (MP5, LR, KNN, SVM, MLP)
\begin{table*}[!h]
\begin{center}
\begin{tabular}{|c|c|c|c|c|c|}
\hline 
...
\hline
  MLP     & 3643.59 $\pm$ 72.40 & 696.92 $\pm$ 85.38 & 1609.37 $\pm$ 76.44  & 108.54 $\pm$ 13.34  & 105.96 $\pm$ 4.93   \\
\hline
\end{tabular}
\end{center}
\caption{\small{Results (errors and time) obtained using the different 
forecasting methods on the dataset with 7 variables per book.}}
\label{tabla:results7} %FERGU: es normal que con menos variables tarde más? Sí, el uso de MATERIA y EDITORIAL hacen que el conjunto sea endiablado.
\end{table*}
...
Taking into account the SVM model obtained results, this method obtains the lower 
%FERGU: "the best errors" no se sabe si es lo mejor o lo peor xDD Pon "worst results" o así
...
using the data set with 13 input variables, but when using the data set with %FERGU: a veces pones dataset: unificarlo
7 input variables (including \emph{EDITORIAL} and \emph{SUBJECT}).
...
and \emph{SUBJECT} might affect the worse results obtained on the final dataset.
