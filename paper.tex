...
predictions, that is reducing the error in estimated sales, results in an enhancement in the whole production process \cite{Fildes2010,Saeed2008}, so not only the costs associated to the book itself are affected.
% (Paloma) I was about to change this second "associated" for "affected" (seems the right thing to do). However, what's after this comma is also confusing, given that we already said that is affects to the whole production process. Maybe who wrote this should re-think what he/she meant to say here and change it.
% Antonio - I guess it was a mistake. I think now it is correct.
...
run can always be made, but this will result in temporary losses due to a lack of supply or in bad marketing for the customers, for instance. However, in practice predicting lower sales than the actual ones is not such a big problem because the new print run is ordered before the inventory is exhausted; 
% (Paloma) I think that the thing that should be highlighted here is that is not big deal because the print run is made _before_ the inventory is exhausted. I mean, the sentence highlights the fact that a new print run is made, and not that maybe the situation where there's any book never happens. 
% Suggestion:  However, in practice predicting lower sales than the actual ones is not such a big problem because the new print run is orderer before the inventory is exhausted;
% Antonio - fixed then
 nevertheless, this new print run presents the same problem as the
initial one of printing only as many as forecasted, although its cost
is not as high as the initial one. 
% (Paloma) Should we really say this next sentence? :\
%and we are not so concerned with
%this issue in this paper.
% Antonio - yes, no need to say this
...
given to the author, or finally distributed in used-books channels such as
...
Our main contribution is, thus, the development of the methodology to process book sales data in order to, first find out relevant variables, and then, use these variables to process the data to accurately estimate future sales. To our knowledge this is the first work in which regression methods have been applied to estimate sales for new launched books. Moreover, the use of a real dataset of book sales is another point to remark with respect to other works.

Therefore, in this paper, the problem of predicting total sales is approached using real data provided by the Spanish publishing company Trevenque Editorial S.L.\footnote{{\tt http://editorial.trevenque.es}}. This company offers management systems and services for bookshops, publishers, and distributors. 
...
The rest of this paper is structured as follows: next, Section \ref{sec:soa} presents a comprehensive review of the approaches found in the bibliography related to sales prediction in similar scopes and shows a set of definitions of the regression methods applied in the paper and the evaluation metrics considered.
...
Book sales forecasting is not a very prolific field of research. There are just a few papers working in this scope \cite{Chang2005_book_forecast_SOM-CBR}...
% Antonio - TODO - completar esto una vez leido el paper con sus partes malas y buenas de nuestro paper, claro
% TODO - buscar si hay m�s trabajos...
...
or fuzzy systems \cite{Mastorocostas2001}, have been applied to solve sales prediction problems in other industrial sectors.
...
Many of these previous papers propose using different prediction methods to find out which one performs the best.
Thus, next subsections describe several forecasting methods and different 
evaluation metrics, commonly used in the bibliography that have been chosen 
to address the problem of book sales prediction.
...
In addition, we think it is not really relevant for the accuracy of the models 
...
The methods to apply are those described in Section \ref{subsec:select_method},  which include rule- and tree-based algorithms, together with numerical regression methods. 

The obtained models for the whole dataset have been evaluated by means of the metrics described in Section \ref{subsec:models_evaluation}. The results are presented in Section \ref{sec:experiments}. Then, the same methodology has been applied to some specific cases of publishing companies in order to prove the value of the proposal (Section \ref{sec:example_cases}).
Specific interpretable models have been created as a final result of the work (Section \ref{subsec:decision_trees}). 


% Antonio - CONCERN - The following section seems to be bad placed now. The Methodology Section is now a bit strange with just this subsection. We should think on moving this section and reorganize/improve the methodology description.
...
Next section provides a comparison between the feature reduction through SOM, and the feature selection techniques that have been described.
...
case of \texttt{publisher} or \texttt{collection} this would imply the definition of
...
as \texttt{collection} and \texttt{binding}, but also when using the datasets with 
...
\begin{figure*}[ht]
\begin{center}
\includegraphics[scale=0.42]{./imgs/prediction_all_publisher_Table5.eps}
\end{center}
...
\end{center}
\caption{Boxplots showing the values obtained predicting \texttt{total\_sales}, using all features, with different regression methods (30 repetitions, 10-fold cross validation). Results obtained for different special publishing companies: UDL929 (the best selling publishing company), UDLW12 (the median selling publishing company), UDL704 (highest number of different titles sold) and UDLU11 (medium number of different books sold).
% Just the features selected by ReliefFAttributeEval method have been considered.
}
\label{fig:attributeAll}
\end{figure*}
% Antonio - CONCERN - The text and the results must be contrasted. If I'm not wrong, I remember that we decided to include here just the results obtained by ReliefFAttributeEval, since they were quite good (maybe the best). But there are plotted apparently the results for all the features. Anyway, in the text, it is said that the process is using all the features, then apply FS. But here just the results for all the features are shown. Why don't we show also the results for the corresponding sets of features selected by each FS method?
% Actually in the previous version there were several tables with these results than have been moved to the Appendix. I think we should include another figure fro them. They show important information. ;)
% we can discuss this point later this morning.
% The thing is that in this plot we are using all features!! (here, no feature selection method has been used)
...
(detailed numeric results can be seen in Tables 
...
% tables have been moved to the Appendix and they have been substituted by the new Figure. Comments (text) have been adapted accordingly  -  [pedro]
%Tables \ref{Table:case1results}, \ref{Table:case2results}, \ref{Table:case3results} and \ref{Table:case4results}. 
...
%  dejamos ésta finalmente: tablas al apéndice + incluimos nueva figura con los datos del RFE.
For clarity purpose, only obtained results after applying the RFE method for the 
four cases are shown in Figure \ref{fig:casesReliefFAttributeEval}
 (detailed numeric results can be shown in 
...
in the appendix of tables at the end of this paper).
This one was chosen since the selected features by RFE method showed in the 
previous experiment the highest predictive power among all models.
As can be shown, RFE reduction has noticeably improved the performance of all 
classifiers specially in the case of the best selling publisher house (i.e. UDL929) 
and the publisher which publishes a wide range of different books (i.e. UDL704). 
...
\begin{figure*}[ht]
\begin{center}
\includegraphics[scale=0.6]{./imgs/attribute_ReliefFAttributeEval-FS.eps}
\end{center}
\caption{Boxplots showing the values obtained predicting \texttt{total\_sales}, considering just the features selected by ReliefFAttributeEval method, with different regression methods (30 repetitions, 10-fold cross validation). Results obtained for different special publishing companies: UDL929 (the best selling publishing company), UDLW12 (the median selling publishing company), UDL704 (highest number of different titles sold) and UDLU11 (medium number of different books sold).
}
