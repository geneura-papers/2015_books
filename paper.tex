...
e-mail: {\tt pacv@ugr.es, amorag@geneura.ugr.es,
  ...
  antares.es@gmail.com, mgarenas@ugr.es, palomacd@ugr.es}
}
...
inventory excess. In this paper, we are tackling the problem of
predicting total sales in order to print the right amount of books and
doing so even before the book has reached the stores.  We have been
working on a data set that includes the complete sales data for books
published in Spain across several years. After an initial exploratory
analysis phase using visualisation techniques, we have applied several
feature selection techniques in order to find out what are the
variables that have more impact on sales, and then used machine
learning to create a model and forecast sales. The initial methodology
was used in a commercial product by Grupo Trevenque, but the
methodology described here improves the running time %with
                                %respect to what?- JJ
and is eventually able to predict sales from pre-publication data with
remarkable accuracy, thus creating a useful tool for publishers, for
whom obtained models can provide a reliable guidance on the decision
process of publishing a book, without human intervention either for
the operation or for interpretation.
...
Publishing a book, in the same way as releasing any other cultural product with a
physical substrate, implies several types of risks and costs due to
...
complicated for completely new products, in the sense that it is an unknown
...
estimations of future sales of the new product is extremely important, so 
that production runs do not exceed predicted sales by too much. 
...
which could forecast the future sales a new book will
achieve, in order to optimise production schedules, improve the publishing
company profits or minimise losses \cite{Zhao2001}, 
being this the main objective of this paper: optimizing sales and
improving profit margins by selling the whole print-run. 

In any case, the accuracy of the predictive method we intend to design is affected
negatively by several factors, related to decisions on purchases, to freight 
traffic, or with the distribution strategy \cite{Little1998}.
...
increasing or decreasing the sales forecast based on season or sales
period.
However, carrying out the analysis and forecasting process as a human expert,
may lead to imprecise predictions which may not take all the variables into account. 
...
predictions from the same dataset \cite{Sanders1994}. A
initial version of this method was developed under contract with that
company and eventually integrated in their product. This paper
represents an improved version of that method based on the same data. 
...
presented.  Section \ref{sec:experiments} reports and analyses the obtained results with a 
...
SOM is a type of unsupervised neural networks inspired by the topographical organization of the sensory cortex of human brain \cite{kohonen1998}. It maps highly dimensional data into a lower dimensional space where similar data patterns are located in more adjacent areas on the grid.

SOM is a very useful analysis tool for visualizing complex data with high number of variables in a simple geometric planes. Therefore, it could be used for different tasks like features reduction and data clustering and visualization.
...
% This is copied from the HIV paper. Please rephrase and adapt to the
% particular venue - JJ
...
% Done - %---  Sam
% Please don't use real text for TODOs- JJ
...
\subsection{Results for all editors}
...
case of editorial or collection this would imply the definition of
...
%***TO DO***: Think if including the graphs (scatter plots).
...
\section{Experiments of special editorial cases}
\label{example_cases}

This section presents further experiments conducted on data concerning four interesting special editorial cases. These editorials were selected as representative cases which could be very interesting for decision makers and managers in the domain. The four cases are described as follows:

\begin{itemize}
    \item Best seller editorial: this case represents the editorial that sells the highest number of copies regardless the title of the books sold. Therefore, it is an example of a case on a higher business scale.
    \item Medium seller editorial: this represents an editorial that sells medium number of copies compared to the other editorials (Medium business scale). 
    \item Highest variation editorial: this is an example of an editorial that follows a strategy in publishing a very wide range of different titles.
    \item Medium varied editorial: this case represents an editorial that has a median rank between the other editorial regarding the variety of titles sold.
\end{itemize}


Table \ref{Table:special_cases} shows number of sold copies and number of titles published by the selected editorials.

\begin{table}
\caption{Description of editorials considered as special cases.
\label{tab:desc_editorials}}
\centering{}%
\begin{tabular}{|r|l|l|l|}
\hline 
 & Different books sold & Total copies sold & Description\\
\hline 
\textit{All editorials} & 6083 & 3201645 & 209 editorials\\
\hline 
\textit{UDL704} & 293 & 58236 & Editorial with most different\\ 
       &     &       & books sold\\
\hline 
\textit{UDLU11} & 180 & 249481 & Editorial with medium number\\ 
       &     &       &  of different books sold\\
\hline 
\textit{UDL929} & 216 & 460752 & Best selling editorial\\
\hline 
\textit{UDLW12} & 97 & 2635 & Medium selling editorial\\
\hline 
\end{tabular}
\label{Table:special_cases}
\end{table}

The experiments applied on the selected special cases follow the same methodology used previously for the whole dataset. First, the forecasting models are applied on each dataset using all features, then, the three feature selection methods (ReliefAttributeEval, CfsSubsetEval and SOM) are used to reduce the features in each case. Finally, the classification models are applied again on the reduce datasets to evaluate their performance.



Case 1-A: Developing prediction models to predict Total sales for
Editorial704 (An editorial with the most number of different books sold) - 293
instances.

\begin{table}
\caption{Predicting Total sales for Editorial704}
\centering{}%
\begin{tabular}{|c|c|c|c|c|c|c|}
\hline 
 &  $R^{2}$  & MAE  & RMSE  & RAE (\%)  & RRSE (\%)  & Time (s)\tabularnewline
...
Editorial-UDLU11 (An editorial with a medium number of different books sold) - 180 instances.

\begin{table}
\caption{Predicting Total sales for Editorial-UDLU11}
\centering{}%
\begin{tabular}{|c|c|c|c|c|c|c|}
\hline 
 & $R^{2}$  & MAE  & RMSE  & RAE (\%)  & RRSE (\%)  & Time (s)\tabularnewline
...
Editorial-UDL929 (The best selling editorial) - 216 instances.

\begin{table}
\caption{Predicting Total sales for Editorial-UDL929 (460752 sales) }
\centering{}%
\begin{tabular}{|c|c|c|c|c|c|c|}
\hline 
 & $R^{2}$  & MAE  & RMSE  & RAE (\%)  & RRSE (\%)  & Time (s)\tabularnewline
...
Editorial-UDLW12 (The median selling editor)- 97 instances.

\begin{table}
\caption{Predicting Total sales for Editorial-UDLW12 (2635 sales) }
\centering{}%
\begin{tabular}{|c|c|c|c|c|c|c|}
\hline 
 & $R^{2}$  & MAE  & RMSE  & RAE (\%)  & RRSE (\%)  & Time (s)\tabularnewline
...
Case 1-A: Developing prediction models to predict Total sales for
Editorial704 with ReliefFAttributeEval feature selection.

\begin{table}[!ht]
\caption{Predicting Total sales for Editorial704 (ReliefFAttributeEval-FS) }
\centering{}%
\begin{tabular}{|c|c|c|c|c|c|c|}
\hline 
 & $R^{2}$  & MAE  & RMSE  & RAE (\%)  & RRSE (\%)  & Time (s)\tabularnewline
...
Case 1-B: Developing prediction models to predict Total sales for
Editorial-UDLU11 with ReliefFAttributeEval feature selection

\begin{table}[!ht]
\caption{Predicting Total sales for Editorial-UDLU11 (ReliefFAttributeEval-FS)}
...
Case 2-A: Developing prediction models to predict Total sales for
Editorial-UDL929 with ReliefFAttributeEval feature selection.

\begin{table}[!ht]
\caption{Predicting Total sales for Editorial-UDL929 (ReliefFAttributeEval-FS)}
...
Case 2-B: Developing prediction models to predict Total sales for
Editorial-UDLW12 with ReliefFAttributeEval feature selection.

\begin{table}[!ht]
\caption{Predicting Total sales for Editorial-UDLW12 (ReliefFAttributeEval-FS)}
...
Case 1-A: Developing prediction models to predict Total sales for
Editorial704 with CfsSubsetEval feature selection.

\begin{table}[!ht]
\caption{Predicting Total sales for Editorial704 (CfsSubsetEval-FS)}
\centering{}%
\begin{tabular}{|c|c|c|c|c|c|c|}
\hline 
 & $R^{2}$  & MAE & RMSE & RAE (\%) & RRSE (\%) & Time (s)\tabularnewline
...
Case 1-A: Developing prediction models to predict Total sales for
Editorial704 with SOM feature selection.

\begin{table}[!ht]
\caption{Predicting Total sales for Editorial704 (SOM-FS)}
\centering{}%
\begin{tabular}{|c|c|c|c|c|c|c|}
\hline 
 & $R^{2}$  & MAE & RMSE & RAE (\%) & RRSE (\%) & Time (s)\tabularnewline
...
Case 1-B: Developing prediction models to predict Total sales for
Editorial-UDLU11 with CfsSubsetEval feature selection.

\begin{table}[!ht]
\caption{Predicting Total sales for Editorial-UDLU11 (CfsSubsetEval-FS)}
\centering{}%
\begin{tabular}{|c|c|c|c|c|c|c|}
\hline 
 & $R^{2}$  & MAE & RMSE & RAE (\%) & RRSE (\%) & Time (s)\tabularnewline
...
Case 1-B: Developing prediction models to predict Total sales for
Editorial-UDLU11 with SOM feature selection.

\begin{table}[!ht]
\caption{Predicting Total sales for Editorial-UDLU11 (SOM-FS)}
\centering{}%
\begin{tabular}{|c|c|c|c|c|c|c|}
\hline 
 & $R^{2}$  & MAE & RMSE & RAE (\%) & RRSE (\%) & Time (s)\tabularnewline
...
Case 2-A: Developing prediction models to predict Total sales for
Editorial-UDL929 with CfsSubsetEval feature selection.

\begin{table}[!ht]
\caption{Predicting Total sales for Editorial-UDL929 (CfsSubsetEval-FS)}
\centering{}%
\begin{tabular}{|c|c|c|c|c|c|c|}
\hline 
 & $R^{2}$  & MAE & RMSE & RAE (\%) & RRSE (\%) & Time (s)\tabularnewline
...
Case 2-A: Developing prediction models to predict Total sales for
Editorial-UDL929 with SOM feature selection.

\begin{table}[!ht]
\caption{Predicting Total sales for Editorial-UDL929 (SOM-FS)}
\centering{}%
\begin{tabular}{|c|c|c|c|c|c|c|}
\hline 
 & $R^{2}$  & MAE & RMSE & RAE (\%) & RRSE (\%) & Time (s)\tabularnewline
...
Case 2-B: Developing prediction models to predict Total sales for
Editorial-UDLW12 with CfsSubsetEval feature selection.

\begin{table}[!ht]
\caption{Predicting Total sales for Editorial-UDLW12 (CfsSubsetEval-FS)}
\centering{}%
\begin{tabular}{|c|c|c|c|c|c|c|}
\hline 
 & $R^{2}$  & MAE & RMSE & RAE (\%) & RRSE (\%) & Time (s)\tabularnewline
...
Case 2-B: Developing prediction models to predict Total sales for
Editorial-UDLW12 with SOM feature selection.

\begin{table}[!ht]
\caption{Predicting Total sales for Editorial-UDLW12 (SOM-FS)}
\centering{}%
\begin{tabular}{|c|c|c|c|c|c|c|}
\hline 
 & $R^{2}$  & MAE & RMSE & RAE (\%) & RRSE (\%) & Time (s)\tabularnewline
