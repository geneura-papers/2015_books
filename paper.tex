...
sales and, taking into account their knowledge of industry, their
...
descriptive and variables are known in advance related to the new book. In
order to do that, we have used a training dataset which includes
historical sales data % (Paloma) so, have we or have we not historical data? It's all contradictory
 for a big amount of books published in Spain for
...
There is not a standard methodology to solve the general problem of sales forecasting. This is why the steps we have followed in this paper are those usually applied in a Knowledge Discovery in Databases (KDD) process \cite{fayyad1996data}. These are described below.
...
sales, for instance. % (Paloma) Why remove those with null values and not to use an algorithm to fill those missing values?
Furthermore, the initial set of variables, presented
...
launched, i.e. \textit{pre-sales data}. % (Paloma) Where are they? In table 2?
...
Once the models and results have been obtained for the whole dataset, the same methodology has been applied to some specific cases of publishing companies in order to prove the value of the proposal.
...
Due to this situation, the initial set of features in the dataset must be revised in order to consider just those that can be known before the launch of the book, i.e. those whose value do not depend on the book sales. Therefore, we have selected the subset of features described in Table \ref{tabla:params_pre_sales}.
