...
%%%%%%%%%%%%%%%% autores %%%%%%%%%%%%%%%
\author {
P.A. Castillo, A.M. Mora, J.J. Merelo, \\ A.J. Fern\'andez-Ares, P. Garc\'{\i}a-S\'anchez, M.G. Arenas
}
...
why in this paper we are tackling the problem or predicting sales to %FERGU: The problem ON predicting, no?
...
%********************************************************************************
\section{Sales forecasting. State of the art} %FERGU: State of the art on sales forecasting?
\label{sec:soa}
...
is perhaps the most used technique to tackle the sales forecasting problems, %FERGU: terminar esta frase con punto en vez de coma para que no sea larga
...
In any case, in this research, historical data from published books is used, %FERGU: a qué research se refiere? A la nuestra o a la de la referencia? Dejarlo más claro, por si acaso
...
decision trees for sales forecasting in textile-appared fashion industry, %FERGU: appared? apparel, creo
...
pattern so that products in the same class would have similar sales pattern. %FERGU: "_a_ similar sales pattern", mejor? 
...
Proposed method is suitable in those sales forecasting problems with abundant %FERGU: _The_ proposed method? 
...
and only some descriptive data about the new book is available. %FERGU: data es plural. is->are
...
Trevenque company, whose expert manager advised this research, provided %FERGU: los párrafos anteriores son un poco dispersos, puedes quitar los puntos y aparte y agruparlos por temas (por ejemplo, todo lo de Trevenque debería de ir en un párrafo)
...
Thus, in this study there are three main stages (see Figure \ref{fig:stages} %FERGU: there are->we propose
for details): 
\begin{itemize}
...
equations\footnote{http://www.gepsoft.com/gxpt4kb/Chapter10/Section3/Introduction.htm} %FERGU: mejor tratar esto como una referencia, ya que es un texto
...
Specifically, in order to compute the forecasting errors of the different methods, %FERGU: quitar este salto de linea y unirlo al párrafo de arriba
...
\emph{EDITORIAL} and \emph{MATTER} were included, while %FERGU: vale, no useis MATTER, usad mejor SUBJECT (MATTER es materia de física, creo xD)
...
\label{tabla:results7} %FERGU: es normal que con menos variables tarde más?
\end{table*}
...
\emph{EDITORIAL} and \emph{MATTER} should be taken into account.
...
Taking into account the SVM model obtained results, this method obtains the best %FERGU: "the best errors" no se sabe si es lo mejor o lo peor xDD Pon "worst results" o así
