...
In this section we show the output of one of the models built during the regression, namely we present two of the decision trees generated by M5P method, to demonstrate the value of the proposal to generate a useful decision-aid tool for and expert (publisher) to see what sales can be expected for the different values of the input variables (including the print-run). The first decision tree is considered for the publisher that sells the most. Figure \ref{fig:decision_tree_best_seller} depicts the decision tree for the best seller publishing company (UDL929). The leaves correspond to linear regression models that can be computed (easily) to get the final predicted value for book sales. The second decision tree is for the same publishing company (the same data, but just considering the selected features by RAE method, since it is the one which yields the best results). The latter tree is shown in Figure \ref{fig:decision_tree_best_seller_fs}.

Examining both decision trees, it can be noticed that the \texttt{tot\_points\_sale} variable appeared twice in the tree and one of them was at the root. This indicates that the total number of points of sale has a major role in forecasting the total sales. We can also notice that \texttt{distrib\_novelty} appears at the top of the trees at a higher level compared to the \texttt{ret\_price} variable. In addition, both tree models reveal that marketing variables such as the total number of points of sale and the number of units distributed as novelty have higher influence on the prediction than the selling price of the book. This interpretation leads us to suggest that companies could probably improve the accuracy of their sales forecasting models by incorporating more marketing related variables in their models. 
