...
%\addto\captionsenglish{\renewcommand{\bibname}{References}} % For book or report class
%\addto\captionsenglish{\renewcommand{\refname}{References}} % For article class

% If the Babel package is NOT used
%\renewcommand{\bibname}{References} % For book or report class
...
Finally, in many cases the predictions are influenced by external variables that must be taken into account, such as seasonality, promotions or fashions \cite{Lapide1999}.
...
This paper faces the new book publishing problem using different prediction methods.

\textbf{Describir en detalle el problema de la tirada editorial.}
...
Para cada libro, se incluye información acerca del precio, lugares donde se han puesto a la venta, materia, editorial, ventas, devoluciones, entre otros datos.
...
\begin{itemize}
  \item  First, after the editorial company has provided the raw data, taken directly from its database, a selection variables process is carried out. In this stage, a SOM cluster method is used to analyze data, to eliminate unrelated variables and to select the best variables to be considered in the forecasted model.
...
\textbf{TODO:} FALTA CONTAR COMO SE HAN ANALIZADO Y TRATADO LOS DATOS PARA REDUCIR EL NUMERO DE ENTRADAS, ETC.


Table \ref{tabla:params43} shows the set of variables describing a book after analizyng the original dataset using the SOM to select the best variables to be considered.

\begin{table}[!h]
\caption{Variables describing a book, after analizyng the original dataset provided by the company Trevenque using the SOM to select the best variables to be considered.}
...
Table \ref{tabla:params13} shows the set of variables describing a book after a second analisys carried out along the company experienced manager to select the best variables to be considered.

\begin{table}[!h]
\caption{Variables describing a book, after a second analisys carried out along the company experienced manager to select the best variables to be considered to solve the sales forecasting problem.}
...
\textbf{TODO:} FALTA CONTAR AQUI LOS METODOS QUE SE HAN UTILIZADO EN ESTE TRABAJO, SUS PARAMETROS, ETC.
 ...
 %  Hemos usado los siguientes 5 métodos de predicción:
