...
associated to the book itself are associated,
...
are made; % (Paloma) I think that the thing that should be highlighted here is that is not big deal because the print run is made _before_ the inventory is exhausted. I mean, the sentence highlights the fact that a new print run is made, and not that maybe the situation where there's any book never happens. 
...
Many of the previous papers propose using different prediction methods.
...
% Antonio - CONCERN - should we change the term 'classification' by 'regression' somehow?
% I don't think so in this case. That is what kNN is...
...
% Antonio - CONCERN - remove this classification thing due to a reviewer's comment 
% ok    :)
...
% Antonio - CONCERN - something about regression?
% in that sentence, approximation is used as synonymous of regression   ;)
...
% Antonio - TODO: link this section with the rest of the SotA. Introduce it in a paragraph before. And rewrite the initial paragraphs to separate it from section 4 (methodology)
% I have addressed this "todo" before (lin. 353)
...
% Antonio - TODO - revise them
% What have to be revised?   :/
...
promotions, that classical prediction methods can not deal with, since they 
are not included in the dataset due to the difficulty to model them. 
...
% Antonio - Should we speak about 'forecasting' or 'regression', Sam? Could they be used with the same meaning?
% I think so.


As stated before, in Section \ref{subsec:select_method}, a selection of the best 
forecasting techniques has been reported, looking for those rule- or tree-based, 
together with numerical regression methods which could yield the best results. 

Once the models and results have been obtained for the whole dataset 
(Section \ref{sec:experiments}), the same methodology has been applied to some 
specific cases of publishing companies in order to prove the value of the 
proposal (Section \ref{sec:example_cases}).
