...
Specifically, the following forecasting methods are proposed: % Proposed why? - JJ
\begin{itemize}
...
All experiments are performed using 10-fold cross validation. The
dataset is partitioned into 10 parts where the training process is
carried out on 9 parts then the resulted model is tested on the left
10th part. This process is repeated 10 times, each time the model is
trained using different 9 parts. At the end of the process the 10
evaluation results on the testing parts are averaged. 
...
%This should go also to exploratory data analysis, that is, problem
%description - JJ
\label{fig:count}
\end{figure*}
...
categories. It can be clearly seen that the SOM algorithm has
successfully clustered the labels of the Total Sales into clear
identified regions. %So what? - JJ
 In the top of the U-matrix the VL category is
...
% OK, this means that these features are more or less clearly
% separable. But the problem is that the UMatrix shows only 2, maybe 3
% clusters: vh and H. That's it. If you know UMatrix, that's what you
% see - JJ
...
%  This is not a result, it is data preprocessing. You should have
%  included it in the 4th section. All results do not need to go into
%  the "REsults" section, only the final results. 
% Besides, are you using this or feature selection for selecting
% features? 
