...
First, after the editorial company has provided the raw data (extracted directly from its database), a preprocessing method has been performed in order to remove incorrect or incoherent patterns, such as those with null values in required variables, or with negative sales, for instance. Moreover, the initial set of variables (presented in Table \ref{tabla:paramsOrig50}) has been reduced by an expert to use just those that can be known in advance when a new book is launched, i.e. \textit{pre-sales data}.

Then, a Feature Selection (FS) process \cite{kittler1986feature} has been carried out, as it is an effective dimensionality reduction technique and an essential preprocessing method to discard `noisy' (non-useful) features \cite{Krishnapuram2004}. The aim of these algorithms is exploring the space of attributes to find out which subset of them yields the best results in a classification or prediction problem.
...
The obtained results are then compared and analysed, considering, in addition to the obtained \textit{correlation coefficient}, some error measurements from the literature \cite{ChingChin2010,Madsen2008}. Those are also called error functions or cost functions. We have considered the widely accepted standard equations \cite{gepsoft} (Equations \ref{eq:MAE} to \ref{eq:RRSE}), which are implemented in tools like Weka \cite{pentaho,Witten2011} 
or R \cite{otexts,Hyndman2013}.
