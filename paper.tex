...
% - se ha usado una máquina Linux, Intel(R) Core(TM) i5-4430 CPU @ 3.00GHz (4 cores), 16 GB RAM
% - 
% - 
% Preparar tests estadísticos para ver si hay diferencias en los errores entre métodos.
% - 
% - 
...
Obtained results using the different forecasting methods on the dataset with 13 
variables per book can be shown in Table \ref{tabla:results13}:
\begin{table*}[!h]
\begin{center}
...
Obtained results using the different forecasting methods on the dataset with 7 
variables per book can be shown in Table \ref{tabla:results7}:
\begin{table*}[!h]
\begin{center}
\begin{tabular}{|c|c|c|c|c|c|}
\hline 
 Method   & Time (sec.)         & MAE     & RMSE    & RAE (\%)  & RRSE (\%)      \\
\hline
  MP5     & 7.77 $\pm$ 1.15     & 117.78  & 260.05  & 18.51     & 17.00  \\
\hline
...
\hline
  MLP     & 3646.76 $\pm$ 64.83 & 705.69 $\pm$ 77.22   & 1608.69 $\pm$ 42.96  & 110.92 $\pm$ 12.14   & 105.18 $\pm$ 2.81   \\
\hline
\end{tabular}
\end{center}
\caption{\small{Results (errors and time) obtained using the different 
...
\textbf{TODO: FALTA COMENTAR LOS RESULTADOS OBTENIDOS CON LOS 5 MÉTODOS.}

% - indicar qué método da mejores resultados
% - indicar qué método es tipo "caja-negra" y cuáles ofrecen modelos "explicables"
% - 
%
% => comienzo anotando pros y contras de los diferentes métodos: 

%M5P model advantages are that it can learn and tackle tasks with high dimensionality
%and it generates reproducible and comprehensible representations that provide 
%information on the decision process and does not require human intervention 
%either for the operation or for interpretation. Thus, it is very suitable in 
%order to be incorporated in a publishing company processes


%Main drawbacks of the Multilayer Perceptron method are both the cost of system 
%building and the fact that it is a back-box model, and thus, it is difficult 
%to explain how the forecasts are obtained.


%The proposed forecasting method to solve the problem of how many books should 
%be printed when a new book is published can be used for products with historical 
...
to forecast new book sales, and from them deduce print-run, based on those variables.
...
standard methods.
Proposed method was validated by the company and incorporated in a business 
intelligence tool for publishing houses.

In this sense, the decision trees proved to be a suitable model because of the ease 
to explain how the prediction is obtained from the variables that describe the new book. 
This, compared to the black-box models such as the ANN was decisive from the point 
of view of the needs of experts and the company.

This research provides a new direction in the sales forecasting area using 
