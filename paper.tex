...
\author[ugr]{Pedro A. Castillo Valdivieso}
\ead{pacv@ugr.es}
...
\author[ugr]{Antonio Javier Fern\'andez-Ares}
\ead{antares.es@gmail.com}
\author[ugr]{Paloma de las Cuevas}
\ead{palomacd@ugr.es}
\author[ugr]{María Isabel García Arenas}
\ead{mgarenas@ugr.es}
...
\address[abd]{Business Information Technology Department, King Abdullah II School for Information Technology, The University of Jordan, Amman, Jordan. Tel: +. Fax: +}
...
them are categorical, which makes them harder to process, even more so if 
...
been generally based on experts' experience.
% Reference - JJ
% [pedro] I have not found a reference. This is based on what Santiago told us.
% We can remove this sentence if you think it only should appear together with a reference...
They analyse data about sales and, taking into account their knowledge of industry, 
their experience, and their perception about trends, could make decisions 
on the companies production, i.e. how many books should be printed when 
...
%
  ...
  %     Moon2010ICSSSM
  %     Moon2010ICEC
Recently, some works have proposed analysing relationships between on-line information and book sales \cite{Moon2010ICSSSM,Moon2010ICEC}. Specifically, Moon et al. propose using the number of blog references as an indicator of
the success of a book, and thus, its sales. Different sales patterns
are observed by analysing the obtained data. 
% Antonio - describir esos 'sales patterns'. ;)
% FERGU: y también cuales son esos 'data obtained'
%The authors considered in these papers historical data from published books, 
...
% (Paloma) Agree with Fergu, I don't think we should say this... Please review it.

% [pedro] La cosa es que esta gente toma libros ya publicados y de los que tienen todos los datos de ventas para analizar y obtener patrones de ventas. Nosotros trabajamos con datos de libros que ni siquiera se han impreso aún, por lo que no tenemos ni idea de cómo van a funcionar. Esta gente dispone de datos de ventas, mientras que nosotros sólo vamos a tener datos relativos a "¿dónde lo vas a poner a la venta?" o "¿vas a dar algún regalo para intentar vender más?" (cosas así).

% Antonio - He intentado escribirlo, pero no me queda claro... Los datos que tenemos son también de libros ya publicados, ¿no?. Estamos considerando variables pre-venta sólamente, eso sí, pero usando datos históricos queremos predecir ventas futuras.
% Quizá lo que habría que señalar (aquí y en los demás sitios que se dice lo de los datos históricos) es lo de las variables que se consideran, si son variables post-venta o pre-venta, como es nuestro caso. ;)
...
\cite{ChingChin2010}. Thus, only pre-sales data are available in this problem. %PABLO: pre-sales data is mentioned briefly in introduction, expand it
...
%
In the same line, Thomassey et al. \cite{SThomassey2014} proposed using a clustering method together with decision trees for sales forecasting in textile-apparel fashion industry, 
a very similar issue to the new book publishing one. % (Paloma) Very similar? Why?
...
Ching et al. propose in \cite{ChingChin2010} a decision-support system for new product sales
...
forecasting methods can be applied, which is what the authors did in
...
on time-series are not adequate, although they can obviously be used once the book has been launched for sale. 
...
The problem to solve in this work is the forecasting of sales for new launched books in the Spanish market, considering a set of `pre-sales variables'.
The objective is to offer a tool for publishers which will aid them to decide the best print run for a new book which before its publication process starts. This is a forecasting problem for which no specific historical data are available, since the same book has not been previously published.
However, we have considered data about other previously published books and have taken as reference a set of variables (or features), related with the book publication process.

The training dataset included historical data about 6083 books published in Spain for several years, with an initial set of 50 variables per pattern. It was provided by the Spanish publishing company Trevenque Editorial S.L.. Table
...
Furthermore, the initial set of variables, presented in Table \ref{tabla:paramsOrig50}, has been reduced by an expert to use just those that can be known before a new book is launched, i.e. \textit{pre-sales data}. Thus, their value do not depend on the book sales. The selected the subset of features is described in Table \ref{tabla:params_pre_sales}.
...
As it can be seen in the table, \texttt{total\_sales} is the output variable, so it is the objective of the prediction methods. On the other hand, \texttt{print\_run} has been considered as an input variable because it is something to be decided in advance by a publisher. In addition, this variable could be interesting to be used to test the prediction model, once it is built. Hence, the publisher could try different values for it and see what the predictor yields, in order to take the best decision.
%Pablo: still waiting the definition of the variables
...
consider it in his or her own prediction process. 
...
There is not a standard methodology to solve the general problem of sales forecasting. This is why the steps we have followed in this paper are those usually applied in a Knowledge Discovery in Databases (KDD) process \cite{fayyad1996data}. These are described in the following sections.

As a summary, after the data has been collected and initially preprocessed (as described in previous section), other preprocessing techniques are applied, such as Feature Selection (FS) \cite{kittler1986feature} (see Section \ref{subsec:data_preproces}). These methods check the relevance of all the variables of the patterns, aiming to reduce even more this set of features, in order to obtain easier-to-interpret models, in a less amount of time (due to their lower complexity), and with a fair or, hopefully, better accuracy.
...
After this process (in Section \ref{subsec:select_method}), a selection of the best forecasting techniques have been conducted, looking for those rule- or tree-based, together with numerical methods which could yield the best results. 
...
The application of FS also aims to reduce the complexity of the obtained forecasting models themselves, which, in turn, implies a reduction in the computational time for building these models. The first has been a key factor in this work, because one of the objectives is to obtain `easily' interpretable decision models, which can be useful for human experts (publishers).
Regarding the algorithm running, it time grows with the number of
features, making it impractical for problems with a large number of features 
\cite{Selvakuberan2008}, but this is a secondary objective in this work.
...
FS is one of the ways of performing a feature reduction, the other one is feature extraction \cite{kittler1986feature}. 
...
In the problem this work addresses, it is very important to maintain the semantics of the set of initial variables, and therefore just FS methods have been applied. 
...
(SOM) \cite{kohonen1998} method has been also applied as a `subjective' (based on expert's opinion) FS technique based on the visualisation of the data as a set of 2D planes (one per feature of the dataset), and their interpretation for finding out relationships between these features.
...
where the samples are scarce. U-Matrix graph, together with the projection of variables into separate planes, the so-called component planes analysis (that SOM yields), will aid an expert to identify the most relevant features on every cluster, i.e. those that can have a higher influence on the creation of that cluster.
...
closest $k$ vectors found in the training set by majority vote.
 ...
 \item \emph{Random Forest}: This method is based on the construction of a set of decision trees using a stochastic process over the basis of C4.5 algorithm. It was proposed by Breiman \cite{Breiman2001} and aims to create independent and uncorrelated trees based on different and random input vectors, following the same distribution.
...
\hline 
K-NN  & 78.7 $\pm$ 0.08  & 272.29 $\pm$ 32.38  & 840.59 $\pm$ 240.11  & 47.48 $\pm$ 3.39  & 63.07 $\pm$ 10.29  &  \textbf{0.001 $\pm$ 0.01}\tabularnewline
\hline 
...
\hline 
K-NN & 86.9 $\pm$0.09 & 231.78 $\pm$24.25 & 668.03 $\pm$205.14 & 40.48 $\pm$3.06 & 51.28 $\pm$16.81 &  \textbf{0.002 $\pm$0.004}\\
\hline 
...
\hline 
K-NN & 84.0 $\pm$0.10 & 289.09 $\pm$22.83 & 743.92 $\pm$260.45 & 50.59 $\pm$3.50 & 57.30 $\pm$22.90 &  \textbf{0.001 $\pm$0.001}\\
\hline 
...
\hline 
K-NN  & 86.8 $\pm$0.09  & 237.91 $\pm$25.34  & 665.64 $\pm$212.05  & 41.55 $\pm$3.14  & 50.96 $\pm$16.72  &  \textbf{0.001 $\pm$0.004}\tabularnewline
\hline 
...
\begin{table}
\caption{Description of publishing companies considered as special cases.Best values in bold.}
\centering{}%
\resizebox{12cm}{!}{
...
\hline 
K-NN  & 73.3 $\pm$0.19  & 1057.21 $\pm$385.04  & 1875.85 $\pm$916.96  & 61.88 $\pm$12.37  & 68.75 $\pm$15.98  &  \textbf{0 $\pm$0.0001}\tabularnewline
\hline 
...
\hline 
K-NN  & 79.3 $\pm$0.24 & 15.34 $\pm$8.71  & 26.57 $\pm$16.64  & 46.20 $\pm$23.77  & 60.09 $\pm$35.21  &  \textbf{0.0001 $\pm$0.0002}\tabularnewline
\hline 
...
\hline 
K-NN  & 82.6 $\pm$0.10  & 87.60 $\pm$23.21  & 158.63 $\pm$54.16  & 47.62 $\pm$10.32  & 59.17 $\pm$14.43  &  \textbf{0.0001 $\pm$0.0003}\tabularnewline
\hline 
...
\hline 
K-NN  & 73.2 $\pm$0.16  & 596.62 $\pm$205.37  & 933.06 $\pm$356.11  & 60.62 $\pm$17.85  & 71.36 $\pm$22.34  &  \textbf{0 $\pm$0.0002}\tabularnewline
\hline 
...
\hline 
K-NN & 87.8 $\pm$0.11 & 751.54 $\pm$270.91 & 1257.82 $\pm$615.66& 44.57 $\pm$10.89 & 48.62 $\pm$18.73 &  \textbf{0.0001 $\pm$0.0003}\tabularnewline
\hline 
...
\hline 
K-NN & 77.2 $\pm$0.30 & 14.18 $\pm$6.63 & 23.65 $\pm$11.03 & 43.37 $\pm$19.23 & 57.00 $\pm$30.59 &  \textbf{0 $\pm$0.0001}\tabularnewline
\hline 
...
\hline 
K-NN  & 85.6 $\pm$0.10  & 78.39 $\pm$24.12  & 147.07 $\pm$58.02  & 42.58 $\pm$10.99  & 54.42 $\pm$15.54  &  \textbf{0.0001 $\pm$0.0003}\tabularnewline
\hline 
...
\hline 
K-NN & 82.7 $\pm$0.09 & 487.00 $\pm$140.27 & 741.81 $\pm$238.56 & 49.27 $\pm$10.50 & 56.62 $\pm$12.59 &  \textbf{0.0001 $\pm$0.0003}\tabularnewline
\hline 
...
In this section we show the output of one of the models built during the regression, namely we present two of the decision trees generated by M5P method applied to the data of the publisher that sells the most. These aim to demonstrate the value of the proposal to generate a useful decision-aid tool for an expert (publisher) to see what sales can be expected for the different values of the input variables (including the print-run).  

First, Figure \ref{fig:decision_tree_best_seller} depicts the decision tree for the publishing company UDL929. The leaves correspond to linear regression models that can be computed (easily) to get the final predicted value for book sales. 
The second decision tree (Figure \ref{fig:decision_tree_best_seller_fs}) corresponds to the same publishing company, so the same data were used, but the RAE feature selection method was applied. Thus, the main difference in the simplification of the regression models in the second tree and the remove of \texttt{print\_run} as part of the decision process after the feature selection.

Examining both trees, it can be noticed that \texttt{tot\_points\_sale} variable appears twice in the trees, being also the root. This indicates that the total number of points of sale has a big influence on the decision models and also a major role in forecasting the total sales. This is an interesting clue for the experts, who could check the expected impact of different number of selling points
According to the trees, \texttt{distrib\_novelty} has also a big repercussion, appearing at the second level of the top of the trees.

An important fact that these tree models reveal is that marketing variables, such as the total number of points of sale and the number of units distributed as novelty have higher influence on the prediction than the selling price of the book, which could be, initially, considered as one of the most important parameters to determine sales. This interpretation leads us to suggest that companies could probably improve the accuracy of their sales forecasting models by incorporating more marketing related variables in their models.  
...
when a new book is going to be published is faced using several data-mining and
forecasting methods.
...
A dataset consisting of 6000 books provided by the Spanish %FERGU: mover este párrafo debajo del siguiente
