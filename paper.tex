...
These FS techniques have been applied in the following section, including a comparison between all of them.
...
MLP also gets a very good correlation coefficient, however it takes the largest time to built the models, and obtains high error measures. ELM shows a good balance, since it is very fast in the training phase (the second faster) and yields good results according to the correlation and the error metrics.
...
We have used both the {\tt CfsSubsetEval} (CSE) \cite{Hall1998} (a correlation-based feature subset evaluation method) and the ReliefFAttributeEval (RFE) \cite{RobnikSikonja1997} (an adaptation of relief for attribute estimation) evaluation methods, implemented in the Weka tool and described in Section \ref{subsec:data_preproces}.
...
four cases are shown in Figure \ref{fig:casesReliefFAttributeEval}
 (detailed numeric results can be seen in 
