...
since it implies a loss of investment. That is why it is an issue to
print the right amount of book, and do so even before the book has
reached shelves. That is the problem we try to solve in this paper.
...
% En la bibliografía se proponen diferentes métodos de predicción de
% ventas, principalmente para la industria de la moda. En esos
% trabajos, cuando las ventas son estables, con una tendencia clara a
% lo largo del tiempo, los métodos clásicos son una buena opción. Sin
% embargo, en el caso de nuevos productos, tales como la publicación
% para su venta de un nuevo libro, los métodos clásicos no funcionan
% bien. 
...
In this paper, which works with data on published books in Spain, we
...
\end{abstract}
 ...
 %El abstract no me gusta nada. Debería ir a la introducción. Hay que
 %enmarcar el problema (hace falta predecir las ventas para no imprimir
 %más de la cuenta) hacer un resumen de lo hecho y decir el
 %resultado. Lo más importante es, ¿cuál es el selling point?
...
The publishing process and individual decisions at each step are very
important. One of those decisions is the number of copies to print. % ¿Esto que tiene que ver con la frase anterior? - JJ
...
% En el ámbito de la industria de la moda se trata de un problema muy
% complejo, sujeto a ciertos requisitos \cite{SThomassey2014} y que se
% ha abordado con diversos métodos en las últimas décadas
% \cite{Thomassey2012,Xia2012,Hammond1990,SThomassey2014}. 
% Pero ¿a santo de qué viene aquí la industria de la moda? Habrá que
% introducirlo diciendo... El problema de la industria similar es
% parecido a otras industrias, etc, etc...

In the field of fashion predicting sales is very complex problem, subject
to certain requirements \cite{SThomassey2014} and has been faced using
various methods in recent decades
\cite{Thomassey2012,Xia2012,Hammond1990,SThomassey2014}.  % Este
                                % párrafo no dice absolutamente nada
                                % ni aporta nada. Si se menciona la
                                % industria de la moda, es que vamos a
                                % aprender algo de ella. ¿Cuáles son
                                % los requisitos? ¿Cuáles son los
                                % métodos? ¿Vamos a aprender de ellos?
                                % - JJ
...
In the field of publishing industry, it is more important printing an
...
enough, you can always print more. Thus, it is extremely important to
forecast the sales a new book might achieve. % Hala, otro salto. ¿Qué
                                % tiene que ver esto con el párrafo
                                % anterior? - JJ
...
Thus, using a predictive tool is very useful when used properly; in
this sense, it is very important to know the product, the
characteristics of sales and how to distribute it, as those details
affect the process \cite{Armstrong2001,SThomassey2014}. % ¿Esto
                                % también pasa en la industria de la
...
them fluctuate.  These factors are usually not controllable and
sometimes unknown. 
Some have to do with decisions on purchases, others with freight
traffic, and others with the distribution strategy
\cite{Little1998}. % ¿Qué hacen todas estas cosas? ¿Vamos a tenerlas
                   % en cuenta? 
...
Publishers generally use historical data and an expert analyzes that
data and applies his knowledge using external variables to obtain quite
appropriate predictions. % ¿cita para esto?  - JJ
However, in recent years the sales are forecasted using an
application. % ¿Cita para esto? - JJ
This is used as the foundation over which an expert applies his
knowledge, as the company might require an interpretable and
understandable model. For example, the expert might increase or
decrease the sales prediction based on season or sales period.
However, the obtained analysis can be imprecise and may not take all
the variables into account. 
In addition, the process is often tedious, especially when the amount
of data is large, and even, different experts can reach different
predictions from the same data set. % We NEED a reference for this!!!
                                % - JJ
...
\bigskip
%---------- estado del arte
% ¿Su propia sección? En ese caso, todo lo del estado del arte de
% arriba debería eliminarse - JJ
...
In the literature several proposals to perform the prediction of sales
in different areas can be found.  %frase que no dice nada. ¿Por qué no
                                %empiezas enmarcando el estado del
                                %arte? Methods to predict book sales
                                %have been generally based on ... - JJ
On the one hand, several qualitative methods are based on an expert
experience, while time series prediction methods are based on using
historical data to make predictions
\cite{ChingChin2010,FaderHardie2005,Madsen2008}. % Hasta ahora no has
                                % dicho cuál es el problema. Tienes
                                % que decir claramente qué tienes y
                                % qué quieres. - JJ
...
Using time series prediction methods is perhaps the most used
technique to tackle the sales forecasting task. 
...
Sales forecasting in editorial industry is a very complex
problem. %hala, vuelta la burra al trigo - JJ
...
Finally, in the literature different error measures can be found to report the results obtained using the prediction methods. 
...
That is why different standard error measurements (Equations \ref{eq:MAE} to \ref{eq:RRSE}), implemented in tools like Weka\footnote{http://wiki.pentaho.com/display/DATAMINING} \cite{Witten2011} or R\footnote{https://www.otexts.org/fpp/2/5} \cite{Hyndman2013}, are proposed.
...
Table \ref{tabla:params13} shows the set of variables describing a book after the second analisys.
	...
	% [Pedro] ¿a qué gráficas te refieres? ¿a las tablas de las variables?
% NO, a fig:som43, creo, porque ha desaparecido. - JJ
\begin{figure}[!h] 
\begin{center}
\begin{tabular}{c}
...
%describir de forma más precisa: visualización usando UMatrix de... -
%JJ
...
% Root relative squared error RRSE
 ...
 % [Pedro] los resultados aún no están terminados de calcular. Pongo
 % unos números cualquiera.
% ¿Qué sentido tiene usar los 4 métodos de error? ¿Es que vamos a
% seleccionar uno? - JJ
...
\hline
  MP5 		& 35 $\pm$ 1 	& 20 $\pm$ 2 	& 20 $\pm$ 2 	& 20 $\pm$ 2 	& 20 $\pm$ 2 	\\
\hline
  LR 		& 35 $\pm$ 1 	& 20 $\pm$ 2 	& 20 $\pm$ 2 	& 20 $\pm$ 2 	& 20 $\pm$ 2 	\\
\hline
  SVM 		& 35 $\pm$ 1 	& 20 $\pm$ 2 	& 20 $\pm$ 2 	& 20 $\pm$ 2 	& 20 $\pm$ 2 	\\
\hline
  KNN 		& 35 $\pm$ 1 	& 20 $\pm$ 2 	& 20 $\pm$ 2 	& 20 $\pm$ 2 	& 20 $\pm$ 2 	\\
\hline
  MLP 		& 35 $\pm$ 1 	& 20 $\pm$ 2 	& 20 $\pm$ 2 	& 20 $\pm$ 2 	& 20 $\pm$ 2 	\\
\hline
\end{tabular}
\end{center}
...
\end{table*}
 ...
 % [Pedro] los resultados aún no están terminados de calcular. Pongo unos números cualquiera.
...
FEDER de Andalucía 2007-2013), TIN2011-28627-C04-02 and
...
GENIL project (CEI-BIOTIC Granada). % ¿Y el contrato con Trevenque?  - JJ
