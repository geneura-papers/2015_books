...
For clarity purpose, obtained results after applying feature selection methods
have been shown in Tables \ref{Table:case1results_no_fs} to \ref{Table:case4results_fs_som}
in the appendix of tables at the end of this paper.
...
% tables have been moved to the Appendix and they have been substituted by the new Figure. Comments (text) have been adapted accordingly  -  [pedro]
%For clarity purpose, we present here only the results after applying the RFE method for the four cases in Tables \ref{Table:case1results-RAE}, \ref{Table:case2results-RAE}, \ref{Table:case3results-RAE} and \ref{Table:case4results-RAE}, this one was chosen since the selected features by this method showed in the previous experiment the highest predictive power among  all classifiers. 
...
% [pedro]  TODO => una vez hecha la nueva figura, volver a meter este comentario anterior (lin.1297)
...
publishing company, so the same data were used, but the RAE % Problem here: everytime you use RAE in the paper it means Relative Absolute Error, so I don't think we should use just "RAE" here, but ReliefFAttributeEval.
feature
